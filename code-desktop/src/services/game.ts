...
export const mainLoop = async (tick: number, context: any, width: number, height: number, windowDimensions: WindowDimensions) => {
    ...
    if (windowDimensions.width === window.innerWidth && windowDimensions.height === window.innerHeight) {
        requestAnimationFrame((tick) => mainLoop(tick, context, width, height, windowDimensions));
    }
}
